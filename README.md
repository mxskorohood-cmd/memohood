...
Без памяти агент помнит только то, что помещается в текущий контекст: начали новую сессию — и всё, что было вчера, исчезло. MemoHood хранит важное отдельно, в локальной базе на вашей машине, и подмешивает нужные кусочки обратно в разговор ровно тогда, когда они пригодились бы. Это провайдер памяти (`memory.provider`) для hermes-agent — устанавливается одной командой и работает полностью автономно.

## От каких проблем избавляет

- **Агент забывает всё, стоит закрыть чат.** → Помнит между сессиями — факты живут в локальной базе, а не в контексте одного диалога.
- **Приходится каждый раз вручную напоминать контекст о себе.** → Авто-recall сам подтягивает нужные факты перед каждым ответом, без просьбы «вспомни».
- **Путает старое решение с новым, держится за устаревшее.** → SUPERSEDE: новый факт наверх, старый помечается устаревшим и хранится с датой — видно и «как сейчас», и «как было».
- **Память жила «в голове» модели и обнулялась при смене модели.** → Факты лежат в отдельной локальной базе, не привязанной к модели: сменил модель — recall работает так же, ничего не теряется.
- **Платишь токенами за обработку пустой болтовни.** → Дешёвый офлайн-гейт (Model2Vec) отсекает трёп до дорогого вызова модели.
...
| Критерий | MemoHood | Без памяти (только system prompt) |
|---|---|---|
| Что помнит | Диалоги: факты, решения, исправления, предпочтения | Только текущий контекст сессии |
| Забывание | Плавное, по кривой Эббингауза; pinned — никогда | Полное при новой сессии или сжатии контекста |
| Противоречивые факты | Supersede: новое замещает старое, старое — в историю | Копятся в контексте, модель путается |
| Стоимость | Доли цента за ход; месячный потолок по умолчанию $5 на провайдера | Ноль |
...
Перезапустите hermes — память уже работает, в keyword-режиме: сигналы вроде «запомни», «решили», «на самом деле» запоминаются бесплатно, без единого внешнего вызова.

Если hermes у вас работает как systemd-сервис (gateway), ключи из `.env` подхватятся после `hermes gateway restart` — установщик уже прописал для этого системный drop-in (`EnvironmentFile`). В интерактивной CLI-сессии `.env` подхватывается сам собой: она наследует окружение процесса, из которого её запустили.

**2. Ключи — по желанию, для семантики.** Без них память работает только на точном совпадении слов; чтобы включить поиск по смыслу (эмбеддинги + реранк) и извлечение спорных фактов, впишите ключи в `.env` рядом с `config.yaml` (`~/.hermes/.env`, на Windows — `%LOCALAPPDATA%\hermes\.env`). Проще всего это сделать мастером настройки прямо в терминале:

```
hermes memohood setup
```

Мастер идёт по шагам, по одному вопросу за раз, и каждый можно пропустить простым Enter (память продолжит работать, просто скромнее): сначала Cloudflare (эмбеддинги — векторный поиск по смыслу), потом Cohere — и тут же простыми словами объясняет, что такое реранкер («строгий редактор», который пересортировывает найденное так, что самое нужное оказывается сверху), затем Gemini (экстракция спорных фактов и ночная консолидация), и напоследок проверяет, что нужные python-библиотеки на месте. По желанию каждый ключ можно сразу проверить одним живым запросом к провайдеру. В консоли ключ целиком никогда не показывается — только первые символы с многоточием — и в таком же замаскированном виде пишется в `.env`. Мастер не трогает `config.yaml` — это уже сделал установщик на шаге 1 (если он запускался без `--no-config`).
...
Cloudflare нужен для векторного поиска (эмбеддинги, бесплатный тир), Gemini — для экстракции спорных фактов и ночной консолидации, Cohere — необязательный реранк. Что произойдёт, если какого-то ключа нет — см. FAQ ниже. Не хотите зависеть от Cloudflare вовсе — переустановите с флагом `--local`/`-Local`: установщик поставит `fastembed`, скачает модель `intfloat/multilingual-e5-large` (~2.2 ГБ, один раз) и сам пропишет офлайн-эмбеддер в `config.yaml`.
...
| `model.model` | string | `gemini-3.1-flash-lite` | Конкретная модель для экстракции и консолидации |
...
Модель видит память как пять инструментов; человек — как команды `hermes memohood`.
...
Запустите установщик (`install.sh`/`install.ps1`, см. «Быстрый старт» выше) — он сам ставит зависимости, копирует плагин и включает `memory.provider: memohood` в конфиге, без единой ручной правки. Ключи (если они вам нужны) собираются отдельно, терминальным мастером `hermes memohood setup` — см. предыдущий вопрос.
